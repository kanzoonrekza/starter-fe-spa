  ...
  return (
    <main className="mx-auto max-w-xl space-y-6 p-6">
      ...
      <p className="text-muted-foreground">
        Reference feature: create an item and load it from either backend.
      </p>
      <form onSubmit={submit} className="space-y-2">
        ...
        <input
          ...
          className="border-input focus-visible:ring-ring w-full rounded-md border p-2 focus-visible:ring-2"
        />
        <Button type="submit" disabled={create.isPending || !name.trim()}>
          {create.isPending ? "Adding…" : "Add item"}
        </Button>
        {create.isError && (
          <p role="alert">{resolveServerMessage(create.error)}</p>
        )}
      ...
      {items.isError && (
        <div role="alert">
          <p>{resolveServerMessage(items.error)}</p>
